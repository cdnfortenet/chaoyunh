...
function Freight(Language){
	...
	var url='http://www.i-oms.cn/#/offeraquery?to=one&chargeWeight='+$('#yf_weight').val()+'&packageType='+$('#yf_package').val()+'&transCategory=&destNo='+$('#yf_country').val()+'&companyNo='+CompanyNo;
	window.open(url,'_blank','');
	return false;
}
function Login(ObjForm){
...
function Reg(Language){
	...
	$.ajax({
		url:regApi,
		type:'post',
		data:{userNo:$('#reg_user').val(),companyNo:CompanyNo},
		dataType:'json',
		success:function(data){
			Loading_Remove(1);
			var code=data.result_code;
